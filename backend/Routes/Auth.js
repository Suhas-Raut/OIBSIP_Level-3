  ...
  async (req, res) => {
    ...
    try {
      ...
      if (user) {
        return res
          .status(400)
          .json({ success, message: "User already exists" });
      }
      ...
      // 🔔 Email sending will come NEXT STEP
...
router.post('/getlocation', async (req, res) => {
    try {
        ...
        console.error(error.message)
        rres.status(500).json({ success: false, error: "Server Error" })
...
router.post('/orderData', async (req, res) => {
    ...
    data = [{ Order_date: req.body.order_date }, ...data];
